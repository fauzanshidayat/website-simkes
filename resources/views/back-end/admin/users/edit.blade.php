...
@section('content')
    <section class="max-w-4xl mx-auto bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-100 mb-6">Edit User</h2>
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <select name="role"
                    ...
                    <option value="pengurus" {{ old('role', $user->role) == 'pengurus' ? 'selected' : '' }}>Pengurus
                    </option>
                </select>
                @error('role')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <div class="flex items-start gap-4">
                    <!-- Preview Lama -->
                    <div
                        class="w-32 h-24 flex items-center justify-center border rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-800">
                        @if ($user->photo)
                            <img id="preview" src="{{ asset('storage/' . $user->photo) }}" alt="Foto Lama"
                                class="object-cover w-full h-full">
                        @else
                            <img id="preview" src="{{ asset('assets/img/no-image.png') }}" alt="Default Foto"
                                class="object-cover w-full h-full">
                        @endif
                    </div>
                    <!-- Input File -->
                    <div class="flex-1">
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Upload foto baru untuk mengganti.</p>
                    </div>
                </div>
                @error('photo')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
